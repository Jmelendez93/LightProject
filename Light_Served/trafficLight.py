...
def customShadowCallback_Update(payload, responseStatus, token):
	...
	if responseStatus == "accepted": 
		...
		if 'name' in payloadDict["state"]["reported"]:
			print("Name: " + str(payloadDict["state"]["reported"]["name"]))
		if 'email' in payloadDict["state"]["reported"]:
			print("Email: " + str(payloadDict["state"]["reported"]["email"]))
		if 'light' in payloadDict["state"]["reported"]:
			print("Light status: " + str(payloadDict["state"]["reported"]["light"]))
		print("~~~~~~~~~~~~~~~~~~~~~~~\n\n")
...
def customShadowCallback_Delta(payload, responseStatus, token):
	...
	print(payloadDict)
	lightP,emailP,nameP="No changes","Previous","Previous"
	
	if 'name' in payloadDict["state"]:
		nameP = payloadDict["state"]["name"]
		print("Name: " + str(nameP))
	if 'email' in payloadDict["state"]:
		emailP = payloadDict["state"]["email"]
		print("Email: " + str(emailP))
	if 'light' in payloadDict["state"]:
		lightP = payloadDict["state"]["light"]
		print("Light status: " + str(lightP))
	print("version: " + str(payloadDict["version"]))
	print("+++++++++++++++++++++++\n\n")

	print("Changes made: Light -" + lightP + " requested by - " + nameP + "(" + emailP + ")")
	#If whole form filled out
	if 'name' in payloadDict["state"] and 'email' in payloadDict["state"] and 'light' in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"name":' + '"' + nameP + '", "email":' + '"' + emailP + '", "light":' + '"' + lightP + '"}}}'
	#If only 1 field entered
	elif 'name' not in payloadDict["state"] and 'email' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"light":' + '"' + lightP + '"}}}'
	elif 'name' not in payloadDict["state"] and 'light' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"email":' + '"' + emailP + '"}}}'
	elif 'email' not in payloadDict["state"] and 'light' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"name":' + '"' + nameP + '"}}}'
	#If only 1 field missing
	elif 'name' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"email":' + '"' + emailP + '", "light":' + '"' + lightP + '"}}}'
	elif 'email' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"name":' + '"' + nameP + '", "light":' + '"' + lightP + '"}}}'
	elif 'light' not in payloadDict["state"]:
		JSONPayload = '{"state":{"reported":{"name":' + '"' + nameP + '", "email":' + '"' + emailP + '"}}}'
	print(JSONPayload)
